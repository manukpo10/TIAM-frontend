...
export function ListaDelMercado({ day: _day, onComplete }: GameProps) {
  ...
  // Results shows the per-item hit/missed/false-positive board first; the
  // completion card (and its "siguiente nivel"/restart buttons) waits for
  // this to become true so the board's own resolution is never skipped or
  // pushed off-screen by the card. No timer — set only by the "ya los vi,
  // continuar" button below, at whatever pace the player needs.
  ...
  return (
    <div className="px-5 pb-5 pt-4 sm:p-7">
      ...
      {/* Board — stays up through results until resultsSeen too, so the
          hit/missed/false-positive badges below are actually visible before
          the board gives way to the completion card. */}
      ...
      {/* Results phase: legend for the badges above + explicit continue —
          waits for the player, no timer, then reveals the completion card. */}
      {phase === 'results' && !resultsSeen && (
        <div className="mt-4">
      ...
      {/* Results phase: progression. Waits for resultsSeen — see the board
          and legend blocks above, which is where the per-item feedback
          actually lives now. */}
      {phase === 'results' && resultsSeen && (
