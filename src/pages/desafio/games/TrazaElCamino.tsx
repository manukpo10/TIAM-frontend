...
export function TrazaElCamino({ day: _day, onComplete }: GameProps) {
  ...
  // en "Hacer otro"), nunca por re-visitar un nivel a mitad de epoch, así
  // "Repetir" devuelve exactamente los mismos caminos.
  const [epochRoutes, setEpochRoutes] = useState(() =>
  ...
  function restartDifferent() {
    restartEpoch()
    setEpochRoutes(LEVELS.map((lvl, i) => shuffle(lvl.routes).slice(0, ROUNDS_PER_LEVEL[i])))
  }
