...
const INSTITUTIONAL_TEXT = '¡Hola! Quiero consultar por el plan Institucional de TIAM para mi institución.'
const institutionalWhatsappLink = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(INSTITUTIONAL_TEXT)}`
...
export function PlataformaPage() {
  ...
  useEffect(() => {
    document.title = 'Software de estimulación cognitiva para profesionales | TIAM Digital Argentina'
    // Un link a /plataforma#planes debe llegar a Precios — si forzáramos
    // scrollTo(0,0) siempre, pelearía contra el salto al ancla del navegador.
  ...
  function handlePricingCta(planId: string) {
    if (planId === 'inst') {
      window.open(institutionalWhatsappLink, '_blank', 'noopener,noreferrer')
      return
    }
