          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {posts.map((post) => (
                <article
                  key={post.slug}
                  className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-md hover:shadow-lg hover:border-tiam-blue/20 transition-[box-shadow,border-color] duration-200"
                >
                  {post.cover && (
                    <img
                      src={post.cover}
                  ...
                  <h2 className="text-lg font-bold text-tiam-navy leading-snug">
                    <Link
                      to={`/blog/${post.slug}`}
